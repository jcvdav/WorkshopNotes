...
Disclaimer: These are just my notes from the DataCamp course. **Not intended for distribution.**
...
stargazer::stargazer(donation_model, type = "html", single.row = T)
```
...
step_model <- step(null_model, scope = list(lower = null_model, upper = full_model), direction = "forward", trace = F)
...
[Slides](Slides/Course2/chapter1.pdf)
...
```{r, echo = FALSE}
unemployment <- readRDS("Data/2Regression/unemployment.rds")
```
...
- Then use `lm()` and `fmla` to fit a linear model to predict female unemployment from male unemployment using the data set `unemploymen`t.
...
# Print it
# unemployment_model
stargazer::stargazer(unemployment_model, type = "html", single.row = F)
```
...
```{r}
bloodpressure <- readRDS("Data/2Regression/bloodpressure.rds")
```
...
stargazer::stargazer(bloodpressure_model, type = "html", single.row = T)
...
- Plot predictions (on the x-axis) versus actual female unemployment rates. Are the predictions near the x=yx=y line?
...
The data frame `unemployment` and the model `nemployment_model` are in the workspace.
...
stargazer::stargazer(mpg_model, type = "html", single.row = T)
```
...
Strictly speaking, `dframe` and `y` aren't used by `kWayCrossValidation`; they are there for compatibility with other `vtreat` data partitioning functions. You can set them both to `NUL`L.
...
```{r, echo = FALSE}
flowers <- read.csv("Data/2Regression/flowers.csv")
```
...
- Use `fmla` and m`odel.matrix()` to create the model matrix for the data frame `flowers`. Assign it to the variable `mmat`.
...
stargazer::stargazer(flower_model, type = "html", single.row = T)
...
```{r}
alcohol <- read.csv("Data/2Regression/alcohol.csv")
```
...
stargazer::stargazer(model_add, model_interaction, type = "html", single.row = T)
```


**https://campus.datacamp.com/courses/supervised-learning-in-r-regression/issues-to-consider?ex=6**
